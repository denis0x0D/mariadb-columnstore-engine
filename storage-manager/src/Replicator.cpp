                                 ...
                                 size_t length)
{
  ...
  if (!resultPair.first)
  {
    ...
    string header = (boost::format("{ \"version\" : \"%03i\", \"max_offset\" : \"%011u\" }") % version %
                     thisEntryMaxOffset)
                        .str();
    std::memcpy(&dataStr[dataStrOffset], header.c_str(), header.length() + 1);
    dataStrOffset += header.length() + 1;
  ...
  {
    dataStr = std::move(resultPair.second);
    dataStr.resize(thisEntryMaxOffset);
