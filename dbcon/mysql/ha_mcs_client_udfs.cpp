...
#define MAXSTRINGLENGTH 50
  ...
  my_bool setddldebuglevel_init(UDF_INIT* initid, UDF_ARGS* args, char* message, const char* funcname)
  {
    if ((args->arg_count != 1) || (args->arg_type[0] != INT_RESULT) ||
  ...
  const char* setddldebuglevel(UDF_INIT* initid, UDF_ARGS* args, char* result, unsigned long* length)
  {
  ...
  void setddldebuglevel_deinit(UDF_INIT* initid)
  {
  }
