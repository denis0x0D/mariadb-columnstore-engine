...
struct ByteStreamProcessor
{
    ByteStreamProcessor(TupleBPS* tbps, vector<boost::shared_ptr<messageqcpp::ByteStream>>& bsv,
                        uint32_t begin, uint32_t end, vector<_CPInfo>& cpv, uint32_t id,
                        vector<vector<RGData>>& rgDatas, RowGroupDL* dlp)
        : tbps(tbps), bsv(bsv), begin(begin), end(end), cpv(cpv), id(id), rgDatas(rgDatas), dlp(dlp)
    {
    }
    ...
    void operator()()
    {
        ...
        tbps->process(bsv, begin, end, cpv, id, rgDatas, dlp);
    }
};
...
void TupleBPS::startProcessingThread(TupleBPS* tbps, vector<boost::shared_ptr<messageqcpp::ByteStream>>& bsv,
                                     uint32_t start, uint32_t end, vector<_CPInfo>& cpv, uint32_t id,
                                     vector<vector<RGData>>& rgDatas, RowGroupDL* dlp)
{
    fProcessorThreads.push_back(
        jobstepThreadPool.invoke(ByteStreamProcessor(tbps, bsv, start, end, cpv, id, rgDatas, dlp)));
}
...
void TupleBPS::process(vector<boost::shared_ptr<messageqcpp::ByteStream>>& bsv, uint32_t begin, uint32_t end,
                       vector<_CPInfo>& cpv, uint32_t id, vector<vector<RGData>>& rgDatas, RowGroupDL* dlp)
{
    // FIXME
    uint32_t threadID = 0;
    ...
    try
    {
        ...
        while (1)
        {
            ...
            for (uint32_t i = 0, e = workSizes.size(); i < e; ++i)
            {
                ...
                startProcessingThread(this, bsv, start, end, cpInfos[i], i, rgDatas, dlp);
                start = end;
            }
