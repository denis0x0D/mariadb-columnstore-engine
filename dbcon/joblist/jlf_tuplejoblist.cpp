                  ...
                  uint32_t prevTable, JoinEdges& joinEdges)
{
    ...
    for (auto sub : joinGraph[currentTable].fAdjacentList)
    {
        ...
        if (joinGraph[sub].fVisited && prevTable != sub)
        {
            ...
            if (!joinEdges.count(edgeForward) &&
                !joinEdges.count(edgeBackward))
            {
        ...
        else if (joinGraph[sub].fVisited == false)
        {
            ...
            collectEdges(joinGraph, jobInfo, sub, currentTable, joinEdges);
        }
    }
}
                 ...
                 JoinEdges& joinEdges)
{
    for (auto& edge : joinEdges)
    {
        if (jobInfo.trace)
        {
            cout << "Remove " << edge.first << " from adjlist of "
                 << edge.second << endl;
            cout << "Remove " << edge.second << " from adjlist of "
                 << edge.first << endl;
        }
...
void initJoinGraph(const TableInfoMap& infoMap,
                                ...
                                JoinEdges& joinEdges)
{
    ...
    collectEdges(joinGraph, jobInfo,
