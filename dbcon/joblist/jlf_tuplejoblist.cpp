...
void collectCycles(JoinGraph& joinGraph, const JobInfo& jobInfo, TableInfoMap& tableInfoMap,
                   uint32_t currentTable, uint32_t prevTable, JoinEdges& edgesToTransform, Cycles& cycles)
{
  // Mark as visited.
  ...
  for (const auto adjNode : joinGraph[currentTable].fAdjacentList)
  {
    ...
    {
      collectCycles(joinGraph, jobInfo, tableInfoMap, adjNode, currentTable, edgesToTransform, cycles);
    }
  }
}
...
void chooseEdgeToTransformOuterJoins(TableInfoMap& infoMap, JobInfo& jobInfo, Cycle& cycle,
                                               ...
                                               JobStepVector& joinSteps)
{
  ...
  while (joinStepIt != end)
  {
    ...
    if (tupleHashJoinStep)
    {
      ...
      {
        std::cout << "Erase matched join step with keys: " << tableKey1 << " <-> " << tableKey2 << std::endl;
...
void breakCyclesAndCollectEdges(TableInfoMap& infoMap, JobInfo& jobInfo, Cycles& cycles,
                                JobStepVector& joinSteps, bool isInnerJoin)
{
  for (auto& cycle : cycles)
  {
    // Just to avoid analyzing empty cycles, if there any of them.
    if (cycle.size() == 0)
      continue;

    JoinEdge edgeForward;
    if (isInnerJoin)
      chooseEdgeToTransformInnerJoins(infoMap, jobInfo, cycle, jobInfo.joinEdgesToRestore, edgeForward);
    else
      chooseEdgeToTransformOuterJoins(infoMap, jobInfo, cycle, joinSteps, edgeForward);

    jobInfo.joinEdgesToRestore.insert(edgeForward);
    ...
    // At the parsing execution plang step CS creates a `TupleHashJoinStep` based on the join graph - for each
...
void collectEdgesAndBreakCycles(TableInfoMap& infoMap, JobInfo& jobInfo, JobStepVector& joinSteps,
                                bool isInnerJoin)
{
  JoinGraph joinGraph;
  ...
  breakCyclesAndCollectEdges(infoMap, jobInfo, cycles, joinSteps, isInnerJoin);
}
...
void spanningTreeCheck(TableInfoMap& tableInfoMap, JobStepVector& joinSteps, JobInfo& jobInfo)
{
  ...
  else if (tableInfoMap.size() > 1)
  {
    ...
    std::cout << "Table join ID: " << std::endl;
    for (auto joinStepIt = joinSteps.begin(); joinStepIt < joinSteps.end(); joinStepIt++)
    {
      auto* tupleHashJoinStep = dynamic_cast<TupleHashJoinStep*>(joinStepIt->get());
      if (tupleHashJoinStep)
      {
        std::cout << "Tables for hash join: " << getTableKey(jobInfo, tupleHashJoinStep->tupleId1())
                  << " <-> " << getTableKey(jobInfo, tupleHashJoinStep->tupleId2()) << std::endl;
        std::cout << "Join id " << tupleHashJoinStep->joinId() << std::endl;
      }
    }
    ...
    {
      const auto isInnerJoin = jobInfo.outerOnTable.size() == 0;
      collectEdgesAndBreakCycles(tableInfoMap, jobInfo, joinSteps, isInnerJoin);
    }
  }
                           ...
                           std::vector<std::string>& traces)
{
  ...
  for (const auto& edge : joinEdgesToRestore)
  {
    auto it = jobInfo.tableJoinMap.find(edge);
    // Edge keys.
...
void tryToRestoreJoinEdges(JobInfo& jobInfo, const std::vector<RowGroup>& smallSidesRG,
                           const RowGroup& largeSideRG, std::vector<std::vector<uint32_t>>& smallKeyIndices,
                           std::vector<std::vector<uint32_t>>& largeKeyIndices, std::vector<bool>& typeless,
                           std::vector<std::string>& traces)
{
  if (!jobInfo.joinEdgesToRestore.size())
    return;

  for (uint32_t i = 0, e = smallSidesRG.size(); i < e; ++i)
  {
    if (tryToRestoreJoinEdges(jobInfo, smallSidesRG[i], largeSideRG, smallKeyIndices[i], largeKeyIndices[i],
                              traces))
    {
      typeless[i] = true;
    }
...
SP_JoinInfo joinToLargeTable(uint32_t large, TableInfoMap& tableInfoMap, JobInfo& jobInfo,
                             vector<uint32_t>& joinOrder, JoinEdges& joinEdgesToRestore)
{
                       ...
                       JobInfo& jobInfo, vector<uint32_t>& joinOrder)
{
  ...
  for (vector<uint32_t>::iterator i = jobInfo.tableList.begin(); i < jobInfo.tableList.end(); i++)
  {
    ...
    if (jobInfo.tablesForLargeSide.count(*i))
      joinStepMap[*i] = make_pair(spjs, 3);
    else if ((*i == largest))
  ...
  for (uint64_t js = 0; js < joins.size(); js++)
  {
    ...
    if (bps || tsas || umstream || (thjs && joinStepMap[large].second < 1))
    {
      ...
      tryToRestoreJoinEdges(jobInfo, thjs->getSmallRowGroups(), thjs->getLargeRowGroup(), smallKeyIndices,
                            largeKeyIndices, typeless, traces);
      thjs->configJoinKeyIndex(jointypes, typeless, smallKeyIndices, largeKeyIndices);
