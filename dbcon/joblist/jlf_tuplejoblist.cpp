...
class CircularJoinGraphTransformer
{
 ...
 protected:
  ...
  // Represetns a collection of cycles.
  Cycles cycles;
  // Represents internal `join graph.`
  JoinGraph joinGraph;
  // Represents edges to erase for each cycle in `join graph`.
  JoinEdges edgesToTransform;
  // Represetns a table to start analysis.
...
{
  // Mark as `GREY` to specify processing node.
                                                              ...
                                                              std::pair<JoinEdge, uint32_t>& resultEdge)
{
  ...
  // cycle, it has a maximum weight among all join edges related to that table.
                           ...
                           uint32_t smallSideIndex)
{
  ...
  for (const auto& joinEdgeWithWeight : joinEdgesToRestore)
  {
    const auto& edge = joinEdgeWithWeight.first;
    auto it = jobInfo.tableJoinMap.find(edge);
    // Edge keys.
                                ...
                                PostJoinFilterKeys& postJoinFilterKeys)
{
  ...
  std::vector<pair<uint32_t, uint32_t>> takenEdges;
  for (const auto& joinEdgeWithWeight : edgesToRestore)
  {
    const auto& edge = joinEdgeWithWeight.first;
