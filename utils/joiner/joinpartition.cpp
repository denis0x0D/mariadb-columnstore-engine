 ...
 , currentPartitionTreeDepth(0)
{
  ...
  */
  ...
  try
  {
    ...
    maxPartitionTreeDepth = std::max(
        static_cast<uint64_t>(1), config->uFromText(config->getConfig("HashJoin", "MaxPartitionTreeDepth")));
  }
  catch (...)
  {
  }
...
int64_t JoinPartition::processSmallBuffer(RGData& rgData)
{
  ...
  if (fileMode)
  {
    ...
    htSizeEstimate += rg.getRowCount() * rg.getColumnCount() * 16;
