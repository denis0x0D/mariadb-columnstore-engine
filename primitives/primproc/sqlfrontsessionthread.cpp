...
namespace exemgr
{
  ...
  void SQLFrontSessionThread::analyzeTableExecute(messageqcpp::ByteStream& bs, joblist::SJLP& jl, bool& stmtCounted)
  {
    ...
    PrimitiveServerThreadPools threadPools;
    jl = joblist::JobListFactory::makeJobList(&caep, fRm, threadPools, false, true);
  ...
  try
  {
      ...
      if (tryTuples)
      {
          ...
          {
            jl = joblist::JobListFactory::makeJobList(&csep, fRm, threadPools, true, true);
            // assign query stats
      ...
      {
        usingTuples = false;
        jl = joblist::JobListFactory::makeJobList(&csep, fRm, threadPools, false, true);
