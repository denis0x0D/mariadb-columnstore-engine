                        ...
                        : IDBFileSystem::getFs(IDBDataFile::BUFFERED))
{
    ...
    auto snappyCompressor = compress::getCompressorByType(fCompressorPool, 2);
    auto lz4Compressor = compress::getCompressorByType(fCompressorPool, 3);

    COMPRESSED_CHUNK_SIZE =
        std::max(snappyCompressor->maxCompressedSize(UNCOMPRESSED_CHUNK_SIZE),
                 lz4Compressor->maxCompressedSize(UNCOMPRESSED_CHUNK_SIZE)) +
        64 + 3 + 8 * 1024;

    fUserPaddings = Config::getNumCompressedPadBlks() * BYTE_PER_BLOCK;
    // Set paddings for each.
    snappyCompressor->numUserPaddingBytes(fUserPaddings);
    lz4Compressor->numUserPaddingBytes(fUserPaddings);
...
//  setvbuf(fileData->fFilePtr, fileData->fIoBuffer.get(), _IOFBF, size);
    ...
    compress::CompressInterface::initHdr(
        ...
        hdrSize);
    compress::CompressInterface::setLBID0(fileData->fFileHeader.fControlData,
                                          lbid);
...
int ChunkManager::updateColumnExtent(IDBDataFile* pFile, int addBlockCount, int64_t lbid)
{
    ...
    compress::CompressInterface::setLBID1(hdr, lbid);
